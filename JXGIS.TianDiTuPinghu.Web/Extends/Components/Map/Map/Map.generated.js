// @hash v3-366EEEDAEAEC91E01F61BC1A17F35B20C3294D6B
    ...
    _createClass(Nav, [{
        ...
        value: function render() {
            ...
            return React.createElement(
                'div',
                { className: 'nav' },
                React.createElement('img', { src: _bl_ + "/Reference/image/logo.png" }),
                React.createElement(antd.Icon, { onClick: function onClick(e) {
                        return _this2.appCenterClick();
                    }, type: 'bars' }),
                React.createElement(antd.Icon, { onClick: function onClick(e) {
                        return _this2.searchClick();
                    }, type: 'search' })
            );
        }
    }]);
...
var Map = function () {
    function Map(el, urlArgs) {
        ...
        var map = L.map(el, {
            ...
            layers: [baseLayers.img.base, baseLayers.img.anno, baseLayers.vec.base, baseLayers.vec.anno],
        ...
        var poiItems = L.geoJSON(null, {
            onEachFeature: function onEachFeature(ft, layer) {
    ...
    _createClass(Map, [{
        ...
        key: 'showMarkerShare',
        value: function showMarkerShare(markerShare) {
        ...
        key: 'getMeasureContent',
        value: function getMeasureContent(layer) {
        ...
        */
        ...
        key: 'activateDrawPoint',
        value: function activateDrawPoint(type) {
            var drawOpts = this.options.draw;
            drawOpts.drawType = type;
            drawOpts.drawPointOn = true;
            if (!this.drawCancelBtn) this.drawCancelBtn = $('.leaflet-draw-actions a').last()[0];
            this.drawCancelBtn && this.drawCancelBtn.click();
            if (!this.drawPointBtn) this.drawPointBtn = $('.leaflet-draw-draw-marker')[0];
        ...
        key: 'toggleDrawPoint',
        value: function toggleDrawPoint(type) {
        ...
        key: 'toggleDrawLine',
        value: function toggleDrawLine(type) {
            var drawOpts = this.options.draw;
            drawOpts.drawType = type;
            ...
            if (drawOpts.drawLineOn) {
                if (!this.drawLineBtn) this.drawLineBtn = $('.leaflet-draw-draw-polyline')[0];
        ...
        key: 'toggleDrawPolygon',
        value: function toggleDrawPolygon(type) {
            var drawOpts = this.options.draw;
            drawOpts.drawType = type;
            ...
            if (drawOpts.drawPolygonOn) {
                if (!this.drawPolygonBtn) this.drawPolygonBtn = $('.leaflet-draw-draw-polygon')[0];
        ...
        key: 'showMarkerResult',
        value: function showMarkerResult(layer) {
            _g.markerPanel.createMarkerOnMap(layer);
        }
    }, {
        key: 'showShareResult',
        value: function showShareResult(layer) {
            this.drawItems.addLayer(layer);
            //分享绑定tooltip
            var share = Share.createShare();
            var popup = L.popup().setContent(share.shareDOM);
        ...
        key: 'showMeasureResult',
        value: function showMeasureResult(layer) {
        ...
        key: 'showBaselayer',
        value: function showBaselayer(type, showAnno) {
        ...
        key: '_POIToGeoJSON',
        value: function _POIToGeoJSON(i) {
            return {
                type: "Feature",
                properties: i,
                geometry: {
        ...
        key: 'POIToGeoJSON',
        value: function POIToGeoJSON(poi) {
            var geoJSON;
        ...
        key: 'showPOI',
        value: function showPOI(item) {
            var ftId = item.FEATUREGUID;
            var icons = this.icons;
            this.getPOILayer(ftId, function (layer) {
        ...
        key: 'getPOILayer',
        value: function getPOILayer(ftId, layerFun, otherLayerFun) {
        ...
        key: 'showPOIDetails',
        value: function showPOIDetails(item) {
            var ftId = item.FEATUREGUID;
            var map = this.map;
            this.getPOILayer(ftId, function (layer) {
        ...
        key: 'createCorrecting',
        value: function createCorrecting(point, poi_id) {
        ...
        key: 'createAreaSearch',
        value: function createAreaSearch(point) {
        ...
        key: 'getPOI',
        value: function getPOI(keyword, categories, pagenumber, center, radius) {
...
function init() {
    ...
    var c_searchPanel = ReactDOM.render(React.createElement(SearchCategoryPanel, { btnClick: function btnClick(e) {
            c_resultspanel.getPOI(true, e, []);
        },
        click: getPOI,
        categories: config.SearchCategory }), getDOMById('searchPanel'));

    var c_appcenter = ReactDOM.render(React.createElement(AppCenter, { map: map, windowHeight: $window.height(), layers: config.Layers }), getDOMById('appcenter'));

    var c_toolbar = ReactDOM.render(React.createElement(Toolbar, { map: map,
        markerLabelPanel: c_appcenter.refs.markerPanel,
        fullExtent: function fullExtent(e) {
            return map.fullExtent();
        },
        clearClick: function clearClick(e) {
            return map.clearAll();
        },
        sharePointClick: function sharePointClick(e) {
            return map.toggleDrawPoint('share');
        },
        shareLineClick: function shareLineClick(e) {
            return map.toggleDrawLine('share');
        },
        sharePolygonClick: function sharePolygonClick(e) {
            return map.toggleDrawPolygon('share');
        },
        measureAreaClick: function measureAreaClick(e) {
            return map.toggleDrawPolygon('measure');
        },
        measureLengthClick: function measureLengthClick(e) {
            return map.toggleDrawLine('measure');
        },
        baseLayerChange: map.showBaselayer,
        annoLayerChange: map.showBaselayer,
        showServices: function showServices(e) {
            c_appcenter.show('2');
        },
        fullScreenDom: $main[0] }), getDOMById('toolbar'));

    c_resultspanel = ReactDOM.render(React.createElement(SearchResultPanel, { map: map, url: "SearchPOI",
        hover: function hover(item) {
            return map.showPOI(item);
        },
        click: function click(item) {
            return map.showPOIDetails(item);
        } }), getDOMById('searchresults'));

    var c_nav = ReactDOM.render(React.createElement(Nav, null), getDOMById('nav'));

    var c_footNav = ReactDOM.render(React.createElement(FooterNav, { navs: config.Navs }), getDOMById('footernav'));

    var c_poiDetails = ReactDOM.render(React.createElement(POIDetailsPanel, { map: map, onHidden: function onHidden(e) {
            if (c_resultspanel.state.hidden && !c_resultspanel.clickHidden) c_resultspanel.show();
